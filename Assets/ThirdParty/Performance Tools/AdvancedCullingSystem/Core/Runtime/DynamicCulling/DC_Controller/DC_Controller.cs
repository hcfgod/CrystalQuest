...
namespace NGS.AdvancedCullingSystem.Dynamic
{
    public class DC_Controller : MonoBehaviour
    {
        ...
        {
            int layer = LayerMask.NameToLayer(GetCullingLayerName());

            if (layer == -1)
            {
                Debug.Log("'ACSCulling' layer not found. You need to select Dynamic Culling object in hierarchy to automatically create it");
                return 0;
            }
