...
float3 ComputeWaterOffset(float3 worldPos)
{
	...
	#endif
	float distanceToCameraNormalized = saturate(GetWorldToCameraDistance(worldPos) * 0.001);
	float3 displacementScaleRelativeToDistance = lerp(float3(0.9, 1, 0.9), float3(0.25, 1, 0.25), distanceToCameraNormalized);
