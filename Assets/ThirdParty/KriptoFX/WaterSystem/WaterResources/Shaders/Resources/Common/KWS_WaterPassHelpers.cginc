...
DECLARE_TEXTURE_UINT (KWS_WaterTexID);
...
{
	return LOAD_TEXTURE(KWS_WaterTexID, GetWaterDepthNormalizedUV(uv) * KW_WaterDepth_TexelSize.zw).x;
}

inline uint GetWaterID(float2 uv, int2 offset)
{
	return LOAD_TEXTURE_OFFSET(KWS_WaterTexID, GetWaterDepthNormalizedUV(uv) * KW_WaterDepth_TexelSize.zw, offset).x;
}
...
{
	float4 res = SAMPLE_TEXTURE(KWS_ScreenSpaceReflectionRT, sampler_linear_clamp, GetScreenSpaceReflectionNormalizedUV(uv));
...
{
	return SAMPLE_TEXTURE(KWS_PlanarReflectionRT, sampler_linear_clamp, refl_uv).xyz;
}
