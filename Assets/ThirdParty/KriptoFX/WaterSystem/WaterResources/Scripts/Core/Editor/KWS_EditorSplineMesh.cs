...
namespace KWS
{
    public class KWS_EditorSplineMesh
    {
        ...
        private SplineScriptableData.SplinePoint _nearPoint;
        private SplineScriptableData.Spline _nearSpline;
        private SplineScriptableData.Spline _selectedSpline;
        ...
        public void DeleteSpline()
        {
            ...
            var splines = WaterInstance.SplineMeshComponent.CurrentSplines;
            if (splines == null || splines.Count == 0) return;

            if (splines.Contains(_selectedSpline)) splines.Remove(_selectedSpline);

            WaterInstance.SplineMeshComponent?.UpdateAllMeshes(WaterInstance);
        }

        public void UpdateSplineParams()
        {
            if (_selectedSpline == null) return;

            _selectedSpline.VertexCountBetweenPoints = WaterInstance.Settings.RiverSplineVertexCountBetweenPoints;
            _selectedSpline.Depth = WaterInstance.Settings.RiverSplineDepth;
            WaterInstance.SplineMeshComponent?.UpdateSelectedMesh(WaterInstance, _selectedSpline);
        }
        ...
        void AddSplineByClick()
        {
            ...
            {
                var spline = new SplineScriptableData.Spline();
                var splinePoint = new SplineScriptableData.SplinePoint(0, hit.point + Vector3.up * WaterInstance.Settings.RiverSplineNormalOffset);
        ...
        SplineScriptableData.SplinePoint GetNearSplinePoint(List<SplineScriptableData.Spline> splines, out SplineScriptableData.Spline nearSpline)
        {
            var mouseToCameraRay = HandleUtility.GUIPointToWorldRay(Event.current.mousePosition);

            SplineScriptableData.SplinePoint nearSplinePoint = null;
            nearSpline = null;
        ...
        void DrawSphereGhostUsingRaycast()
        {
            if (KWS_EditorUtils.MouseRaycast(out var hit))
            {
                var position = hit.point + Vector3.up * WaterInstance.Settings.RiverSplineNormalOffset;
        ...
        Vector3 DrawMoveHandle(Vector3 position, Color color)
        {
            var newPos = position;
            Handles.color = color;
            var fmh_189_53_638295391105494964 = Quaternion.identity; newPos = Handles.FreeMoveHandle(newPos, GetHandleSphereSize(position), Vector3.one, Handles.CylinderHandleCap);
            return newPos;
        }
        ...
        Vector3 DrawMoveArrows(Vector3 position, Vector3 forward)
        {
            ...
            position = Handles.Slider(position + Vector3.up * 0.15f, forward) - Vector3.up * 0.15f;
        ...
        void AddNewPoint()
        {
            ...
            if (KWS_EditorUtils.MouseRaycast(out var hit))
            {
                ...
                var newPoint = new SplineScriptableData.SplinePoint(splineID, position);
        ...
        private void DrawHandles(List<SplineScriptableData.SplinePoint> points, bool isSplineSelected)
        {
        ...
        private void UpdatePointPositionIfAllowed(List<SplineScriptableData.SplinePoint> points, Vector3 newPos, int i)
        {
            var isAllowedPrev = i - 1 < 0 || IsAllowedPositionBeetweenPoints(points[i - 1].WorldPosition, newPos);
            var isAllowedNext = i + 1 >= points.Count || IsAllowedPositionBeetweenPoints(points[i + 1].WorldPosition, newPos);
        ...
        private void DrawBezier(List<SplineScriptableData.SplinePoint> points, bool isSplineSelected)
        {
        ...
        public void DrawSplineMeshEditor(Object target)
        {
            ...
            WaterInstance.UndoProvider.Splines = splines;
            Undo.RecordObject(WaterInstance.UndoProvider, "Changed spline point");
            ...
            var badVertices = WaterInstance.SplineMeshComponent.EditorBadVertices;
