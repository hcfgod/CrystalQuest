...
namespace KWS
{
    ...
    /// </code>
    /// </exemple>
    public class KWS_BufferedRTHandleSystem : IDisposable
    {
        Dictionary<int, KWS_RTHandle[]> m_RTHandles = new Dictionary<int, KWS_RTHandle[]>();

        KWS_RTHandleSystem m_RTHandleSystem = new KWS_RTHandleSystem();
        ...
        /// <returns>The frame RT or null when the <paramref name="bufferId"/> was not previously allocated (<see cref="KWS_BufferedRTHandleSystem.AllocBuffer(int, Func{KWS_RTHandleSystem, int, KWS_RTHandle}, int)" />).</returns>
        public KWS_RTHandle GetFrameRT(int bufferId, int frameIndex)
        {
        ...
        public void AllocBuffer(
            int bufferId,
            Func<KWS_RTHandleSystem, int, KWS_RTHandle> allocator,
            int bufferCount
        )
        {
            var buffer = new KWS_RTHandle[bufferCount];
            ...
            for (int i = 1, c = buffer.Length; i < c; ++i)
            {
                ...
                m_RTHandleSystem.SwitchResizeMode(buffer[i], KWS_RTHandleSystem.ResizeMode.OnDemand);
            }
        }
        ...
        /// <summary>
        /// Swap buffers Set the reference size for this RT Handle System (<see cref="KWS_RTHandleSystem.SetReferenceSize(int, int, bool)"/>)
        /// </summary>
        ...
        void Swap()
        {
            foreach (var item in m_RTHandles)
            {
                ...
                if (item.Value.Length > 1)
                {
                    ...
                    m_RTHandleSystem.SwitchResizeMode(item.Value[0], KWS_RTHandleSystem.ResizeMode.Auto);
                    m_RTHandleSystem.SwitchResizeMode(item.Value[1], KWS_RTHandleSystem.ResizeMode.OnDemand);
                }
                else
                {
                    m_RTHandleSystem.SwitchResizeMode(item.Value[0], KWS_RTHandleSystem.ResizeMode.Auto);
