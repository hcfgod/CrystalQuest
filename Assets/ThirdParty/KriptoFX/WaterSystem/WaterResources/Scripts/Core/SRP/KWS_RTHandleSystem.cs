...
namespace KWS
{
    ...
    /// </summary>
    public partial class KWS_RTHandleSystem : IDisposable
    {
        ...
        HashSet<KWS_RTHandle> m_AutoSizedRTs;
        KWS_RTHandle[] m_AutoSizedRTsArray; // For fast iteration
        HashSet<KWS_RTHandle> m_ResizeOnDemandRTs;
        ...
        /// </summary>
        public KWS_RTHandleSystem()
        {
            m_AutoSizedRTs = new HashSet<KWS_RTHandle>();
            m_ResizeOnDemandRTs = new HashSet<KWS_RTHandle>();
            m_MaxWidths = 1;
            m_MaxHeights = 1;
        }
        ...
        public void Release(KWS_RTHandle rth)
        {
            if (rth != null)
            {
        ...
        internal void Remove(KWS_RTHandle rth)
        {
        ...
        internal void SwitchResizeMode(KWS_RTHandle rth, ResizeMode mode)
        {
        ...
        void DemandResize(KWS_RTHandle rth)
        {
            ...
            if (sizeChanged)
            {
                ...
                // Generate a new name
                rt.name = KWS_SPR_CoreUtils.GetRenderTargetAutoName(
                    rt.width,
                    rt.height,
                    rt.volumeDepth,
                    GraphicsFormatUtility.GetGraphicsFormat(rt.format, false),
                    rt.dimension,
                    rth.m_Name,
                    mips: rt.useMipMap,
                    enableMSAA: rth.m_EnableMSAA,
                    msaaSamples: (KWS.MSAASamples)rt.antiAliasing,
        ...
        void Resize(int width, int height, bool sizeChanged)
        {
            ...
            for (int i = 0, c = m_AutoSizedRTsArray.Length; i < c; ++i)
            {
                ...
                renderTexture.name = KWS_SPR_CoreUtils.GetRenderTargetAutoName(renderTexture.width, renderTexture.height, renderTexture.volumeDepth,
                                                                               renderTexture.graphicsFormat,
                                                                               renderTexture.dimension, rth.m_Name, mips: renderTexture.useMipMap,
                                                                               enableMSAA: rth.m_EnableMSAA, msaaSamples: (MSAASamples)renderTexture.antiAliasing,
                                                                               dynamicRes: renderTexture.useDynamicScale);
        ...
        )
        {
            ...
            RenderTexture rt;
            if (isShadowMap || useDepthOnlyFormat)
            {
                ...
                rt = new RenderTexture(width, height, (int)depthBufferBits, format, RenderTextureReadWrite.Linear)
                {
                    ...
                    vrUsage = vrUsage,
                    name = KWS_SPR_CoreUtils.GetRenderTargetAutoName(width, height, slices, format, name, mips: useMipMap, enableMSAA: enableMSAA, msaaSamples: msaaSamples)
                };
            }
            else
            {

                rt = new RenderTexture(width, height, (int)depthBufferBits, colorFormat, mipMapCount)
                {
                    hideFlags = HideFlags.HideAndDontSave,
                    volumeDepth = slices,
                    filterMode = filterMode,
                    ...
                    memorylessMode = memoryless,
                    vrUsage = vrUsage,
                    name = KWS_SPR_CoreUtils.GetRenderTargetAutoName(width, height, slices, colorFormat, dimension, name, mips: useMipMap, enableMSAA: enableMSAA, msaaSamples: msaaSamples, dynamicRes: useDynamicScale)
                };
            }
            ...
            var newRT = new KWS_RTHandle(this);
        ...
        // Internal function
        KWS_RTHandle AllocAutoSizedRenderTexture(
            int width,
            int height,
            int slices,
            DepthBits depthBufferBits,
            bool useDepthOnlyFormat,
        ...
        public KWS_RTHandle Alloc(RenderTexture texture)
        {
            var rth = new KWS_RTHandle(this);
        ...
        /// <param name="texture">Input texture</param>
        /// <returns>A new RTHandle referencing the input texture.</returns>
        public KWS_RTHandle Alloc(Texture texture)
        {
            var rth = new KWS_RTHandle(this);
        ...
        public KWS_RTHandle Alloc(RenderTargetIdentifier texture)
        {
        ...
        /// <returns>A new RTHandle referencing the input render target identifier.</returns>
        public KWS_RTHandle Alloc(RenderTargetIdentifier texture, string name)
        {
            var rth = new KWS_RTHandle(this);
            rth.SetTexture(texture);
            rth.m_EnableMSAA = false;
