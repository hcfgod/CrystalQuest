...
namespace KWS
{
    public class KW_PyramidBlur
    {
        ...
        KWS_RTHandle[] tempBuffersDown = new KWS_RTHandle[kMaxIterations];
        KWS_RTHandle[] tempBuffersUp = new KWS_RTHandle[kMaxIterations];

        private KWS_RTHandle _tempSeparableRT;
        ...
        public void ComputeBlurPyramid(float blurRadius, KWS_RTHandle source, KWS_RTHandle target, CommandBuffer cmd, Vector2 sourceScale)
        {
            ComputeBlurPyramid(blurRadius, source, target, cmd, sourceScale, null);
        }

        public void ComputeBlurPyramid(float blurRadius, KWS_RTHandle source, KWS_RTHandle target, CommandBuffer cmd, ScaleFunc sourceScaleFunc)
        {
            ComputeBlurPyramid(blurRadius, source, target, cmd, Vector2.zero, sourceScaleFunc);
        }

        void ComputeBlurPyramid(float blurRadius, KWS_RTHandle source, KWS_RTHandle target, CommandBuffer cmd, Vector2 sourceScale, ScaleFunc sourceScaleFunc)
        {
            if (_blurMaterial == null) _blurMaterial = CreateMaterial(BlurShaderName);

            var targetRT = target.rt;
            KWS_RTHandle last = source;
            ...
            for (var level = 0; level < iterations; level++)
            {
                ...
                {
                    //tempBuffersDown[level] = KWS_RTHandles.AllocVR(lastWidth, lastHeight, colorFormat: targetRT.graphicsFormat);
                    if (sourceScaleFunc == null) tempBuffersDown[level] = KWS_RTHandles.AllocVR(sourceScale,                colorFormat: targetRT.graphicsFormat);
                    else tempBuffersDown[level]                         = KWS_RTHandles.AllocVR(_targetScaleFuncArr[level], colorFormat: targetRT.graphicsFormat);
            ...
            for (var level = iterations - 1; level >= 0; level--)
            {
                ...
                {
                    //tempBuffersUp[level] = KWS_RTHandles.AllocVR(tempBuffersDown[level].rt.width * 2, tempBuffersDown[level].rt.height * 2, colorFormat: targetRT.graphicsFormat);
                    if (sourceScaleFunc == null) tempBuffersUp[level] = KWS_RTHandles.AllocVR(tempBuffersDown[level].scaleFactor * 2f, colorFormat: targetRT.graphicsFormat);
                    else tempBuffersUp[level]                         = KWS_RTHandles.AllocVR(_targetScaleFuncArr[level - 1],          colorFormat: targetRT.graphicsFormat);
        ...
        public void ComputeSeparableBlur(float blurRadius, KWS_RTHandle source, KWS_RTHandle target, CommandBuffer cmd, Vector2 sourceScale)
        {
            ComputeSeparableBlur(blurRadius, source, target, cmd, sourceScale, null);
        }

        public void ComputeSeparableBlur(float blurRadius, KWS_RTHandle source, KWS_RTHandle target, CommandBuffer cmd, ScaleFunc sourceScaleFunc)
        {
            ComputeSeparableBlur(blurRadius, source, target, cmd, Vector2.zero, sourceScaleFunc);
        }


        void ComputeSeparableBlur(float blurRadius, KWS_RTHandle source, KWS_RTHandle target, CommandBuffer cmd, Vector2 sourceScale, ScaleFunc sourceScaleFunc)
        {
            if (_tempSeparableRT == null)
            {
                if (sourceScaleFunc == null) _tempSeparableRT = KWS_RTHandles.AllocVR(sourceScale, colorFormat: target.rt.graphicsFormat);
                else _tempSeparableRT = KWS_RTHandles.AllocVR(sourceScaleFunc, colorFormat: target.rt.graphicsFormat);
            }
            if (_blurMaterial == null) _blurMaterial = CreateMaterial(BlurShaderName);
