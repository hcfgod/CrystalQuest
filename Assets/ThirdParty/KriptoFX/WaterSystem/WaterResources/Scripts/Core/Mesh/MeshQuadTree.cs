...
namespace KWS
{
    public class MeshQuadTree
    {
        ...
        public class QuadTreeChunksData
        {
            ...
            public List<Mesh> UnderwaterMeshes = new List<Mesh>();
            ...
            public void Release()
            {
                ...
                foreach (var underwaterMesh in UnderwaterMeshes) KW_Extensions.SafeDestroy(underwaterMesh);
                if (VisibleChunksComputeBuffer != null) VisibleChunksComputeBuffer.Release();
                VisibleChunksComputeBuffer = null;

                Instances.Clear();
                InstancesArgs.Clear();
                UnderwaterMeshes.Clear();
        ...
        public void Initialize(QuadTreeTypeEnum quadTreeType, Bounds bounds, Vector3 waterPos, Vector3 waterRotation, WaterSystem.WaterMeshQualityEnum meshQuality, bool useTesselation)
        {
            ...
            var maxBoundSize = Mathf.Max(bounds.size.x, bounds.size.z);
            var underwaterScale = Mathf.Lerp(0.5f, 8, Mathf.Clamp01(Mathf.Max(1, maxBoundSize) / 50f));
            ...
            for (int lodIndex = 0; lodIndex < _lastQuadTreeChunkSizesRelativeToWind.Length; lodIndex++)
            {
                ...
                var underwaterResolution = new Vector2Int((int)(underwaterScale * currentResolution.x), (int)(underwaterScale * currentResolution.y));

                var instanceMesh = MeshUtils.GenerateInstanceMesh(currentResolution, _quadTreeType);
                //var instanceMesh = GenerateSkirt(currentResolution);
                var underwaterMesh = MeshUtils.GenerateUnderwaterMesh(underwaterResolution);

                QuadTreeData.Instances.Add(instanceMesh);
                QuadTreeData.InstancesArgs.Add(null);
                QuadTreeData.UnderwaterMeshes.Add(underwaterMesh);
            }
        ...
        Vector2Int GetChunkLodResolution(QuadTreeTypeEnum quadTreeType, Bounds bounds, int lodIndex, bool useTesselation)
        {
            ...
            {
                chunkRes = Vector2Int.one * quarterRes;
            }
            else
            {
                if (bounds.size.x < bounds.size.z) chunkRes = lodRes * new Vector2Int(Mathf.CeilToInt(bounds.size.x / bounds.size.z), 1);
                else chunkRes = lodRes * new Vector2Int(1, Mathf.CeilToInt(bounds.size.z / bounds.size.x));
        ...
        List<float> InitialiseFiniteLodDistances(Vector3 size)
        {
            ...
            var maxLevelsRelativeToSize = Mathf.CeilToInt(Mathf.Log(maxSize / 4f, 2));
        ...
        internal class Node
        {
            ...
            internal ChunkVisibilityEnum UpdateVisibleNodes(MeshQuadTree root)
            {
                ...
                if (root._quadTreeType == QuadTreeTypeEnum.Infinite)
                {
                    ...
                    if (!KW_Extensions.IsBoxVisibleAccurate(ref root._currentCameraFrustumPlanes, ref root._currentCameraFrustumCorners, min, max))
                ...
                else
                {
                    ...
                    var min = cubeSize - halfSize;
                    var max = cubeSize + halfSize;
                    if (!KW_Extensions.IsBoxVisibleApproximated(ref root._currentCameraFrustumPlanes, min, max)) return ChunkVisibilityEnum.NotVisibile;
