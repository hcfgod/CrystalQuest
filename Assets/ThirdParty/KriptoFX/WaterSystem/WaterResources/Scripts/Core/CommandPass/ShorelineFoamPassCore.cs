...
namespace KWS
{
    ...
    {
        public Action<CommandBuffer, Camera, RenderTexture> OnSetRenderTarget;
        ...
        {
            KW_Extensions.SafeDestroy(_targetRT);
        ...
        void InitializeRT(WaterSystem waterInstance)
        {
            ...
            _targetRT = new RenderTexture(size.x, size.y, 0, format) { name = "_foamRT", enableRandomWrite = true, dimension = dimension, vrUsage = vrUsage, volumeDepth = slices };
            _targetRTSize = new Vector4(_targetRT.width, _targetRT.height, 1.0f / _targetRT.width, 1.0f / _targetRT.height);
        }
        ...
        void InitializeComputeData(WaterSystem waterInstance)
        {
            ...
            _computeFoamBuffer = KWS_CoreUtils.GetOrUpdateBuffer<uint>(ref _computeFoamBuffer, _targetRT.width * _targetRT.height * stereoPasses);
            _computeFoamLightBuffer = KWS_CoreUtils.GetOrUpdateBuffer<uint>(ref _computeFoamLightBuffer, _targetRT.width * _targetRT.height * stereoPasses);

            if (_foamComputeShader == null)
            {
                _foamComputeShader = (ComputeShader)Resources.Load($"PlatformSpecific/KWS_ShorelineFoamParticles");
            ...
            _renderToTextureDispatchSize = new Vector2Int(Mathf.CeilToInt(_targetRT.width / 256f), Mathf.CeilToInt(_targetRT.width / 1f));
        ...
        void RenderFoam(Camera cam, CommandBuffer cmd, RenderTargetIdentifier colorRT, RenderTargetIdentifier depthRT)
        {
            ...
            var stereoPasses = WaterSystem.IsSinglePassStereoEnabled ? 2 : 1;
            if (_targetRT.volumeDepth != stereoPasses) return;
