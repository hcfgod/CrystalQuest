...
namespace KWS
{
    public class ScreenSpaceReflectionPassCore : WaterPassCore
    {
        ...
        class PassData
        {
            public KWS_RTHandle[] ReflectionRT = new KWS_RTHandle[2];
            public KWS_RTHandle   ReflectionRTFilteredMip;
            ...
            public void ReInitializeTextures(WaterSystem waterInstance)
            {
                ...
                {
                    for (int idx = 0; idx < 2; idx++) ReflectionRT[idx] = KWS_RTHandles.AllocVR(ScaleFunc, name: "_reflectionRT" + idx, colorFormat: colorFormat, enableRandomWrite: true);
                    ReflectionRTFilteredMip = KWS_RTHandles.AllocVR(ScaleFunc, name: "_reflectionRT_Mip", colorFormat: colorFormat, useMipMap: true, autoGenerateMips: true, mipMapCount: 5);
                }
                else
                {
                    for (int idx = 0; idx < 2; idx++)
                        ReflectionRT[idx] = KWS_RTHandles.AllocVR(ScaleFunc, name: "_reflectionRT" + idx, colorFormat: colorFormat,
