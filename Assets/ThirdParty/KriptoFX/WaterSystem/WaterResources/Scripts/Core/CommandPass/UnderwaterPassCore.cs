...
namespace KWS
{
    ...
    {
        public Action<CommandBuffer, RenderTargetIdentifier> OnSetRenderTarget;
        ...
        void InitializeTextures()
        {
            ...
            _underwaterRT = KWS_RTHandles.AllocVR(_rtScale, name: "_underwaterRT", colorFormat: hdrFormat);
            _underwaterRTBlured = KWS_RTHandles.AllocVR(_rtScale, name: "_underwaterRT_Blured", colorFormat: hdrFormat);
        ...
        public void Execute(Camera cam, CommandBuffer cmd, RenderTargetIdentifier colorBuffer)
        {
            ...
            if (WaterInstance.Settings.UseUnderwaterBlur)
            {
                ...
                var targetViewPortSize = KWS_CoreUtils.GetCameraRTHandleViewPortSize(cam, WaterSystem.IsSinglePassStereoEnabled);
                ...
                OnSetRenderTarget?.Invoke(cmd, colorBuffer);
                cmd.BlitTriangle(destRT, destRT.rtHandleProperties.rtHandleScale, colorBuffer, targetViewPortSize, _underwaterMaterial, 1);
            }
            else
            {
                cmd.DrawProcedural(Matrix4x4.identity, _underwaterMaterial, 0, MeshTopology.Triangles, 3);
            }
        }
